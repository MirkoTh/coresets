...
tbl_inb <- tbl_x_ii %>% 
  filter(
    category %in% c(0) |
      (category %in% c(1) & x1 %in% seq(1, 9, by = 2) & x2 %in% seq(2, 10, by = 2))
  )
...
plot_grid(tbl_inb_upsample %>% mutate(category = factor(category)))
...
# 1. run gcm with bias parameter varying from unbiased to very biased and plot probabilities category 0 over 100 grid points
# 2. run gcm with upsampled data sets (few added points - many added points)

# main remaining issue: how is upsampling exactly achieved in the model? given I want to add n data points to one category
# possible solution: do upsampling as currently implemented in upsample function, and then rank sampled data points according to their importance for the categorization problem
